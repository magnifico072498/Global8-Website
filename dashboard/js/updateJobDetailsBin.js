function updateJobDetailsBin(button) {
    const jobNo = button.getAttribute("update-data-jobno-bin");
    ...
    `;
                ...
                form.onsubmit = (e) => {
                    e.preventDefault();
                    saveJobDetailsBin(jobNo, new FormData(form));
                };
            } else {
...
function saveJobDetailsBin(jobNo, formData) {
    // Log the form data for debugging
    for (const [key, value] of formData.entries()) {
        console.log(key + ": " + value);
    }

    fetch(`updateJobDetailsBin.php?jobNo=${jobNo}`, {
        method: 'POST',
        body: formData,
    })
        .then(response => response.json())
